...
app = Flask(__name__)
@app.route('/')
def home(): return "4Protect V2 + Snoway - ON"
...
@bot.event
async def on_ready():
    print(f"Connecté: {bot.user} | Fusion 4Protect+Snoway")
    await bot.tree.sync()
...
class HelpSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        if self.values[0]=="Protect":
            e = discord.Embed(title="🛡️ Protect - 4Protect V2", description="```/antilink on/off\n/antispam on/off\n/antibot on/off\n/antichannel on/off\n/antirole on/off\n/antiban on/off\n/antikick on/off\n/antiraid on/off```\nBypass si whitelist/admin")
